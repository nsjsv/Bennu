//! SQLite 预览交互状态与消息处理：自 app-ui 的
//! preview_state/sqlite.rs 迁入（impl FileBrowser → impl PreviewEngine，
//! 逐字节保真；命令构造改为 PreviewMessage）。
...
use crate::sqlite_preview::{
    SqlQueryOutcome, SqliteDatabasePreview, SqlitePreviewMessage, SqlitePreviewTab, SqliteTableData,
};
...
pub struct SqlitePreviewState {
    pub path: PathBuf,
    pub generation: u64,
    pub active_tab: SqlitePreviewTab,
    pub selected_table: Option<String>,
    pub table_loading: bool,
    pub table_data: Option<SqliteTableData>,
    pub sql_text: String,
    pub sql_running: bool,
    pub sql_result: Option<Result<SqlQueryOutcome, String>>,
    pub table_filter: String,
    pub tables_width: f32,
}

/// 表列表默认宽度；拖动分隔条后随会话保持。
pub const SQLITE_DEFAULT_TABLES_WIDTH: f32 = 240.0;
const SQLITE_TABLES_MIN_WIDTH: f32 = 140.0;
const SQLITE_DATA_MIN_WIDTH: f32 = 320.0;

/// 表列表宽度拖动：记录起点，移动时按指针位移调整宽度。
#[derive(Debug, Clone, Copy)]
pub struct SqliteTablesResizeDrag {
    cursor_start_x: f32,
    width_start: f32,
}

fn sqlite_table_data_command(
    path: PathBuf,
    table: String,
    generation: u64,
) -> Task<PreviewMessage> {
    Task::perform(
        ...
        move |outcome| {
            PreviewMessage::SqlitePreview(SqlitePreviewMessage::TableDataLoaded(
                generation, outcome,
            ))
        },
    )
...
fn sqlite_sql_command(path: PathBuf, sql: String, generation: u64) -> Task<PreviewMessage> {
    Task::perform(
        crate::sqlite_preview::run_sqlite_sql(path.clone(), sql),
        move |outcome| {
            PreviewMessage::SqlitePreview(SqlitePreviewMessage::SqlFinished(generation, outcome))
        },
    )
}

impl super::PreviewEngine {
    pub fn start_sqlite_preview(
        &mut self,
        path: PathBuf,
    ...
    pub fn handle_sqlite_preview_message(
        &mut self,
    ...
    pub fn active_sqlite_preview_mut(&mut self) -> Option<&mut SqlitePreviewState> {
        if matches!(
            self.preview,
    ...
    pub fn clear_sqlite_preview(&mut self) {
        self.sqlite_preview = None;
    }

    /// 拖动起点的横坐标由宿主传入（指针位置归宿主输入状态所有）。
    pub fn start_sqlite_tables_resize_drag(&mut self, cursor_start_x: f32) -> Task<PreviewMessage> {
    ...
    pub fn update_sqlite_tables_resize_drag(&mut self, position: Point) {
    ...
    pub fn finish_sqlite_tables_resize_drag(&mut self) -> Task<PreviewMessage> {
