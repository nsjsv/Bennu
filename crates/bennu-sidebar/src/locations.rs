...
pub struct SidebarLocation {
    pub label: String,
    pub path: PathBuf,
    pub kind: SidebarLocationKind,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SidebarLocationKind {
    Home,
    Desktop,
    Documents,
    Downloads,
...
impl SidebarLocationKind {
    pub fn is_user_favorite(self) -> bool {
        matches!(
            self,
            Self::Desktop
                | Self::Documents
...
pub fn sidebar_icon_symbol(location: &SidebarLocation) -> IconSymbol {
    match location.kind {
...
pub fn home_sidebar_location(home: &Path) -> SidebarLocation {
    SidebarLocation {
        label: "Home".to_owned(),
...
pub fn sidebar_favorite_configs(locations: &[SidebarLocation]) -> Vec<SidebarFavoriteConfig> {
    locations
        .iter()
...
/// 从路径推导收藏标签（配置缺 label 时的回退）；config 解析同一规则，
/// 两处共用本实现避免漂移。
pub(crate) fn sidebar_favorite_label_from_path(path: &Path) -> String {
    path.file_name()
        .map(|name| name.to_string_lossy().into_owned())
...
mod tests {
    ...
    #[test]
    fn icon_symbol_follows_location_kind() {
        let symbol = |kind| {
            sidebar_icon_symbol(&SidebarLocation {
                label: String::new(),
                path: PathBuf::new(),
                kind,
            })
        ...
        assert_eq!(symbol(SidebarLocationKind::Home), IconSymbol::House);
        assert_eq!(symbol(SidebarLocationKind::Downloads), IconSymbol::Download);
        assert_eq!(symbol(SidebarLocationKind::Bookmark), IconSymbol::Bookmark);
    }
}
