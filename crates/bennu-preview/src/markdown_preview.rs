//! Markdown 预览渲染面板：自 app-ui view/markdown_preview.rs 下沉。
//! 事件滚动接线（smooth-scroll 包装/id/视口回传）经 ScrollRegionWiring
//! 由宿主注入（宿主滚动管线分工见 scrollbar-guidelines），其余逐字节保真。
...
use crate::text_preview::TextPreviewLineLimitNotice;
...
pub fn markdown_preview_body<Message>(
    markdown: &str,
    ...
    Message: 'static,
{
    ...
    let scroller = scrollable(smooth_scroll_wrap(content.into()))
        .id(scrollable_id)
...
fn markdown_block_view<Message: 'static>(block: MarkdownBlock) -> Element<'static, Message> {
    match block {
...
fn markdown_list_item_view<Message: 'static>(
    depth: usize,
...
fn markdown_code_block_view<Message: 'static>(
...
fn markdown_quote_view<Message: 'static>(
    depth: usize,
    content: String,
) -> Element<'static, Message> {
    row![
