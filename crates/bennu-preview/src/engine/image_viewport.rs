//! 图片预览缩放/平移交互：自 app-ui 的 preview_state/image_viewport.rs
//! 迁入（impl FileBrowser → impl PreviewEngine，逐字节保真）。呈现面
//! 视口尺寸由宿主传入（预览窗口/右侧面板两种表面的几何归宿主）。

use iced::Size;
use iced::Task;

use crate::image_preview_viewport::{image_preview_zoom_multiplier, PreviewImageViewportMessage};
use crate::preview::{image_preview_size, PreviewContent, PreviewSize, PreviewState};
...
impl super::PreviewEngine {
    ...
    pub fn update_preview_image_viewport(
        &mut self,
        message: PreviewImageViewportMessage,
        panel_size: PreviewSize,
    ) -> Task<PreviewMessage> {
