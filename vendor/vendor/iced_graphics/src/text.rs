...
pub fn align(
    ...
    if needs_relayout {
        ...
        // `set_align` 丢弃了 shaping 缓存，而 cosmic-text 0.15 的
        // `set_size` 在尺寸未变时是 no-op（不会重新 shape）。此时
        // `layout_runs()` 会因 `shape_opt() == None` 提前终止，导致
        // 渲染出空文本。与上游 master 一致，这里无条件强制重 shape。
