...
use super::{
    ...
    PreviewFileSizeKind, PreviewFileSizeLimits, UserConfig, ViewDensityLevel, CONFIG_FILE_NAME,
};
...
pub(super) fn parse_toml_user_config(content: &str, default: UserConfig) -> UserConfig {
    ...
    // favorites/network 两段解析用 bennu-sidebar 共享实现（portal 同源，
    // 禁止双份 schema）。
    if let Some(favorites) = bennu_sidebar::parse_toml_sidebar_favorites(&document) {
        config.sidebar_favorites = Some(favorites);
    }
    config.network_connections = bennu_sidebar::parse_toml_network_connections(&document);
