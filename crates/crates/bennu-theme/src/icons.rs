...
pub fn file_entry_icon_symbol(kind: FileKind, name: &OsStr) -> IconSymbol {
    file_kind_icon_symbol(kind, Path::new(name).extension().and_then(OsStr::to_str))
}

pub fn preview_entry_icon_symbol(kind: FileKind, name: &str) -> IconSymbol {
    file_kind_icon_symbol(kind, Path::new(name).extension().and_then(OsStr::to_str))
}

pub fn rotated_chevron_right_view(rotation_degrees: f32, size: f32) -> Svg<'static, Theme> {
...
/// 遮挡产生层次。
pub fn file_kind_solid_symbol(kind: FileKind) -> IconSymbol {
    match kind {
...
impl IconSymbol {
    pub fn view(self, size: f32) -> Svg<'static, Theme> {
    ...
    pub fn bytes(self) -> &'static [u8] {
        match self {
            ...
            Self::Grid => GRID_ICON,
            Self::Ghost => include_bytes!("../assets/icons/lucide/ghost.svg"),
            Self::GhostX => include_bytes!("../assets/icons/lucide/ghost-x.svg"),
...
// lucide "refresh-cw"：设备列表手动刷新按钮。
const REFRESH_ICON: &[u8] = br#"<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>"#;
