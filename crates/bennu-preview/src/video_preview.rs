...
// VideoPreviewFrame 原经 app-ui 的 model 门面引用；下沉后 preview 模块
// 是本 crate 顶层模块，直接按 crate 路径取。
use crate::preview::VideoPreviewFrame;
use crate::preview_message::PreviewMessage;

pub const VIDEO_PREVIEW_MAX_EDGE: u32 = 720;
...
pub async fn inspect_video_preview_metadata(path: PathBuf) -> Result<VideoPreviewMetadata, String> {
...
pub async fn load_video_preview_frame(
    path: PathBuf,
    generation: u64,
    position: Duration,
...
// 流式订阅：ffmpeg 帧流按播放节拍推送（自 app-ui 订阅桥迁入，逐字节
// 保真；输出统一为 PreviewMessage，宿主经 Subscription::map 包装进各自
// 顶层 Message）。
pub fn video_preview_subscription(
    path: PathBuf,
    generation: u64,
...
fn video_preview_stream(
    ...
    iced::stream::channel(2, async move |mut output| {
        ...
        let message = match outcome {
            Ok(()) => PreviewMessage::VideoPreviewFinished(path.clone(), generation),
            Err(error) => PreviewMessage::VideoPreviewFailed(path.clone(), generation, error),
        };
...
async fn stream_video_preview(
    ...
    while let Some(frame) = read_ppm_frame(&mut stdout).await? {
        ...
        output
            .send(PreviewMessage::VideoPreviewFrameLoaded(VideoPreviewFrame {
                path: path.clone(),
                generation,
                position,
                handle: image::Handle::from_rgba(frame.width, frame.height, frame.pixels),
