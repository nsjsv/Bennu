//! Bennu 共享主题库：主程序（app-ui）与 portal-backend（FileChooser）共用
//! 的唯一主题来源。持有 Material 角色模型（UiColorRoles）、主题优先级裁决
//! （ApplicationTheme::active）、内置预设、自定义配色、matugen 文件解析，
//! 以及共享的 lucide 图标与样式词汇（styles）。消费者各自组合。
...
impl ThemeMode {
    pub const ALL: [Self; 3] = [Self::Light, Self::Dark, Self::Automatic];
...
pub use catalog::{ColorSchemeFamily, ColorSchemePreset};
pub use custom::{default_custom_color_scheme, ContrastWarnings, CustomColorScheme};

#[derive(Debug, Clone, Copy, PartialEq)]
pub struct UiColorRoles {
...
impl ApplicationTheme {
    pub fn new(system_fallback: Theme) -> Self {
        Self {
            system_fallback,
    ...
    pub fn active(&self, theme_mode: ThemeMode, color_scheme: ColorSchemePreset) -> Theme {
    ...
    pub fn effective_mode(&self, theme_mode: ThemeMode) -> AppearanceMode {
    ...
    pub fn replace_system_fallback(&mut self, theme: Theme) {
        self.system_fallback = theme;
    }

    pub fn replace_matugen_override(&mut self, theme: Option<Theme>) {
        self.matugen_override = theme;
    }

    pub fn replace_custom_color_scheme(&mut self, custom_color_scheme: CustomColorScheme) {
        self.custom_color_scheme = custom_color_scheme;
    }
}

pub fn fallback_theme(mode: AppearanceMode) -> Theme {
...
pub fn ui_colors(theme: &Theme) -> UiColorRoles {
    UiColorRoles::from_theme(theme)
}

pub fn parse_matugen_theme(document: &str) -> Result<Theme, String> {
...
pub async fn read_matugen_theme_file(path: &Path) -> Result<Option<Theme>, String> {
...
pub fn parse_hex_color(value: &str, field: &str) -> Result<Color, String> {
