...
/// 解码与输出之间的小队列：解码线程先行至多 2 帧，播放侧按节拍消费。
...
#[derive(Debug, Clone)]
pub struct AnimatedImagePreview {
    path: PathBuf,
...
pub enum AnimatedImagePlayback {
    Static,
    Animated,
}
...
impl AnimatedImagePreview {
    ...
    pub fn current_frame_handle(&self) -> &iced_image::Handle {
        &self.current_frame
    }

    pub fn previous_frame_handle(&self) -> Option<&iced_image::Handle> {
    ...
    pub fn stream_start_position(&self) -> Duration {
    ...
    pub fn accept_frame(&mut self, frame: AnimatedImageFrame) {
    ...
    pub fn commit_seek(&mut self, generation: u64) {
...
pub fn is_animated_image_preview_path(path: &Path) -> bool {
    path.extension()
        .is_some_and(|extension| extension.eq_ignore_ascii_case("gif"))
}

// 首帧加载自 app-ui 命令层随迁：超限文案依赖的 format_file_size 已下沉，
// 阻塞解除（订阅流仍留宿主）。
pub async fn load_animated_image_preview(
    path: PathBuf,
    generation: u64,
    max_file_bytes: u64,
...
pub fn animated_image_frame_deadline(
...
// 以下两个解码入口供 app-ui 侧的首帧加载与流式订阅复用（原私有函数，
// 下沉后跨 crate 调用改为 pub）。
pub fn decode_animated_image_frame_at(
    path: &Path,
    generation: u64,
    position: Duration,
...
pub fn decode_animated_image_frames_into_channel(
    path: &Path,
    generation: u64,
    start_position: Duration,
...
pub fn inspect_animated_image_metadata(path: &Path) -> Result<AnimatedImageMetadata, String> {
    let file = File::open(path)
        .map_err(|error| format!("could not open animated image preview: {error}"))?;
...
pub struct AnimatedImageMetadata {
    pub duration: Option<Duration>,
    pub playback: AnimatedImagePlayback,
}
...
// 流式订阅：动图帧按播放节拍推送（自 app-ui 订阅桥迁入，逐字节保真；
// 输出统一为 PreviewMessage，宿主经 Subscription::map 包装进各自顶层
// Message）。
pub fn animated_image_preview_subscription(
    path: PathBuf,
    generation: u64,
    start_position: Duration,
...
fn animated_image_preview_stream(
...
) -> impl Stream<Item = PreviewMessage> + 'static {
    ...
    iced::stream::channel(2, async move |mut output| {
        ...
        let message = match outcome {
            Ok(()) => PreviewMessage::AnimatedImagePreviewFinished(path.clone(), generation),
            Err(error) => {
                PreviewMessage::AnimatedImagePreviewFailed(path.clone(), generation, error)
            }
        };
...
async fn stream_animated_image_preview(
    ...
    loop {
        ...
        while let Some(frame) = frame_receiver.recv().await {
            ...
            output
                .send(PreviewMessage::AnimatedImageFrameLoaded(frame))
                .await
