...
pub struct AudioPreviewMetadata {
    pub duration: Option<Duration>,
    pub len: u64,
}

pub async fn inspect_audio_preview_metadata(path: PathBuf) -> Result<AudioPreviewMetadata, String> {
...
pub async fn start_audio_preview(path: PathBuf) -> Result<AudioPreviewRuntime, String> {
    start_audio_preview_at(path, Duration::ZERO).await
}

pub async fn start_audio_preview_at(
    path: PathBuf,
    position: Duration,
