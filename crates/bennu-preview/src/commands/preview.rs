...
use crate::preview_config::PreviewExtensionRules;
use crate::preview_loading::{load_directory_preview_children, load_preview};
...
pub fn preview_command(
    ...
    Task::perform(
        ...
        move |preview_outcome| PreviewMessage::PreviewLoaded(preview_path.clone(), preview_outcome),
    )
}

/// 右侧面板信息区元数据:符号链接本身不展开(与属性窗口一致),
/// 读失败(已删除/无权限)以 Err 回传,信息区降级为占位。
pub fn right_preview_panel_info_command(path: PathBuf) -> Task<PreviewMessage> {
    ...
    Task::perform(
        ...
        move |snapshot| PreviewMessage::RightPreviewPanelInfoLoaded {
...
fn read_right_preview_panel_info(
...
) -> Result<crate::preview::RightPreviewPanelInfoSnapshot, String> {
    ...
    Ok(crate::preview::RightPreviewPanelInfoSnapshot {
        name: path
            .file_name()
...
pub fn remote_preview_cache_command(
    source_path: PathBuf,
    generation: u64,
    cache_dir: PathBuf,
    max_file_bytes: u64,
...
pub fn preview_directory_children_command(
    path: PathBuf,
    options: ScanOptions,
) -> Task<PreviewMessage> {
    ...
    Task::perform(
        ...
        move |children_outcome| {
            PreviewMessage::PreviewDirectoryChildrenLoaded(parent_path.clone(), children_outcome)
        },
    )
}

pub fn image_preview_dimensions_command(path: PathBuf, generation: u64) -> Task<PreviewMessage> {
    let image_path = path.clone();
    Task::perform(load_image_dimensions(path), move |dimensions| {
        PreviewMessage::ImagePreviewDimensionsLoaded(image_path.clone(), generation, dimensions)
    })
}

pub fn original_image_preview_command(
    path: PathBuf,
    generation: u64,
    max_file_bytes: u64,
    ...
    Task::perform(
        ...
        move |outcome| {
            PreviewMessage::OriginalImagePreviewLoaded(image_path.clone(), generation, outcome)
        },
    )
}
...
pub fn animated_image_preview_command(
    path: PathBuf,
    generation: u64,
    max_file_bytes: u64,
) -> Task<PreviewMessage> {
    ...
    Task::perform(
        ...
        move |preview_outcome| {
            PreviewMessage::AnimatedImagePreviewLoaded(
                image_path.clone(),
                generation,
                preview_outcome,
            )
...
pub fn text_preview_chunk_command(request: TextPreviewChunkRequest) -> Task<PreviewMessage> {
...
pub fn start_audio_preview_command(path: PathBuf) -> Task<PreviewMessage> {
    let audio_path = path.clone();
    Task::perform(start_audio_preview(path), move |playback_outcome| {
        PreviewMessage::AudioPreviewStarted(audio_path.clone(), playback_outcome)
    })
}

pub fn start_video_preview_audio_command(
    path: PathBuf,
    generation: u64,
    position: Duration,
) -> Task<PreviewMessage> {
    ...
    Task::perform(
        ...
        move |audio_outcome| {
            PreviewMessage::VideoPreviewAudioStarted(video_path.clone(), generation, audio_outcome)
        },
    )
}

pub fn video_preview_metadata_command(path: PathBuf) -> Task<PreviewMessage> {
    let video_path = path.clone();
    Task::perform(
        async move {
        ...
        move |metadata_outcome| {
            PreviewMessage::VideoPreviewMetadataLoaded(video_path.clone(), metadata_outcome)
        },
    )
}

pub fn video_preview_frame_command(
    path: PathBuf,
    generation: u64,
    position: Duration,
) -> Task<PreviewMessage> {
    ...
    Task::perform(
        ...
        move |frame_outcome| match frame_outcome {
            Ok(frame) => PreviewMessage::VideoPreviewFrameLoaded(frame),
            Err(error) => PreviewMessage::VideoPreviewSeekFrameFailed(
                video_path.clone(),
                generation,
                position,
                error,
