...
#[derive(Debug)]
pub struct DocumentPreviewWorkspace {
    root: TempDir,
    pdf_path: PathBuf,
}

impl DocumentPreviewWorkspace {
    pub fn create_for_pdf(pdf_path: PathBuf) -> Result<Self, io::Error> {
    ...
    pub fn page_output_prefix(&self, key: &DocumentPageRequestKey) -> PathBuf {
...
#[derive(Debug)]
pub struct OfficeDocumentPreviewWorkspace {
    root: TempDir,
    profile_url: Url,
    output: PathBuf,
...
impl OfficeDocumentPreviewWorkspace {
    ...
    pub fn xdg_config_dir(&self) -> &Path {
        &self.xdg_config
    }

    pub fn xdg_cache_dir(&self) -> &Path {
        &self.xdg_cache
    }

    pub fn xdg_data_dir(&self) -> &Path {
        &self.xdg_data
    }

    pub async fn output_directory_identity_is_current(&self) -> io::Result<bool> {
        let metadata = tokio::fs::symlink_metadata(&self.output).await?;
        Ok(DirectoryIdentity::from_metadata(&metadata) == self.output_identity)
    }

    pub async fn into_document_workspace(
        self,
        pdf_path: PathBuf,
    ...
    #[cfg(test)]
    pub fn private_directories(&self) -> Vec<PathBuf> {
        vec![
